...
// Warm, elegant palette — gold / cream / maroon — matches the deeni-romantic
// aesthetic of the rest of the page instead of a generic rainbow burst.
...
/* ---------- Small decorative crescent + star motif ---------- */
...
/* ---------- Subtle Islamic geometric pattern (very low opacity) ---------- */
...
/* ---------- Step wrapper ---------- */
function Step({ children, onNext, nextLabel = "Continue", isLast = false }) {
  return (
...
function BismillahStep() {
  return (
    <div style={styles.bismillahWrap}>
      ...
      <div style={styles.bismillahArabic}>
        بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ
      </div>
      <div style={styles.bismillahDivider} />
      <p style={styles.bismillahUrdu}>
        Har achi shuruat Uske naam se hoti hai
      </p>
...
function CharacterCardStep() {
  return (
    <div style={styles.characterWrap}>
      ...
      <p style={styles.characterCaption}>
        Aur is kahani ki heroine — sirf tum ho.
      </p>
    </div>
  );
}

/* ---------- Stage 3 — From This to This ---------- */
function FromThisToThisStep() {
  return (
    <div style={styles.transformWrap}>
      <div style={styles.transformRow}>
        <div style={styles.imageCol}>
          <img src={image1} alt="Before" style={styles.transformImg} loading="lazy" />
        </div>

        <div style={styles.arrowCol} aria-hidden="true">
          <svg viewBox="0 0 60 24" width="44" height="20" style={styles.arrowSvg}>
        ...
        <div style={styles.imageCol}>
          <img src={image2} alt="After" style={styles.transformImg} loading="lazy" />
        </div>
      </div>
...
function FriendsCornerStep() {
  return (
    <div style={styles.friendsWrap}>
      <p style={styles.friendsIntro}>
        And the people who walked beside us — they saw it too.
      </p>
...
function DeenStep() {
  ...
  return (
    <div style={styles.deenWrap}>
      ...
      <div style={styles.tasbeehHolder}>
  ...
  return (
    <div style={styles.istikharaWrap}>
      ...
      <div style={styles.istikharaHadithBox}>
        <p style={styles.istikharaHadithArabic}>
...
function SadagiStep() {
  return (
    <div style={styles.sadagiWrap}>
      <svg viewBox="0 0 400 200" width="100%" height="auto" style={styles.calligraphySvg} aria-label="Muqadas in Urdu">
        <defs>
      ...
      <p style={styles.sadagiLine}>
        Kuch log mehngi cheezon se khoobsurat lagte hain,
      </p>
      <p style={styles.sadagiLine}>
...
/* ---------- Stage 8 — Proposal ---------- */
function ProposalStep({ onAccepted }) {
  ...
  if (response === "yes") {
    return (
      <div style={styles.proposalWrap}>
        ...
        <p style={styles.yesLine}>
          Alhamdulillah. Ab ye kitaab hum dono milkar likhenge.
        </p>
        <p style={styles.yesSub}>
          Muqadas, tumhara "Haan" meri zindagi ka sab se haseen lafz hai.
        </p>
  ...
  return (
    <div style={styles.proposalWrap}>
      ...
      <p style={styles.proposalLine}>
        Muqadas, kya tum meri zindagi ka hissa banogi?
      </p>
      ...
      <div style={styles.proposalBtns}>
        ...
        <button
          type="button"
          style={{ ...styles.thinkBtn, transform: `translate(${btnOffset.x}px, ${btnOffset.y}px)` }}
          onMouseEnter={() => {
...
export default function EnvelopeReveal({ children, sealColor = "#c0392b" }) {
  ...
  const storySteps = useMemo(
    () => [
      ...
      { key: "bismillah", next: "Continue", render: () => <BismillahStep /> },
      { key: "character", next: "Continue", render: () => <CharacterCardStep /> },
      { key: "transform", next: "Continue", render: () => <FromThisToThisStep /> },
      { key: "friends", next: "Continue", render: () => <FriendsCornerStep /> },
      { key: "deen", next: "Continue", render: () => <DeenStep /> },
      { key: "istikhara", next: "Continue", render: () => <IstikharaStep /> },
      { key: "sadagi", next: "Continue", render: () => <SadagiStep /> },
      { key: "forever", text: "And I hope it never ends… until Jannah, insha'Allah.", next: "Ek Aakhri Baat…" },
      { key: "proposal", next: null, isLast: true, render: () => <ProposalStep onAccepted={handleAccepted} /> },
    ],
    [handleAccepted]
  );
    ...
    <div style={styles.stage}>
      <style>{`
        ...
        @keyframes bismillahGlow {
          ...
          50%      { text-shadow: 0 0 22px rgba(192,57,43,0.35),
                                 0 0 44px rgba(224,179,84,0.25); opacity: 1; }
        }
        ...
        @media (max-width: 480px) {
          .mobile-page { padding: 32px 20px !important; }
        }
      `}</style>
      ...
      {showPage && (
        <div
          ...
          style={{ ...styles.page, animation: "pageTurn 0.7s ease both" }}
        >
          {children ? (
            children
          ) : (
            <Step onNext={handleNext} nextLabel={current.next || "Continue"} isLast={isLast}>
              {current.render ? current.render() : current.text}
            </Step>
          )}
        </div>
      )}
...
const styles = {
  ...
  bismillahUrdu: { fontSize: "clamp(15px, 3.8vw, 19px)", fontStyle: "italic", color: "#4a4a4a", margin: 0, animation: "fadeIn 1.2s ease both" },
  ayahText: { fontSize: "clamp(13px, 3.2vw, 15px)", fontStyle: "italic", color: "#5a5a5a", lineHeight: 1.7, maxWidth: 500, margin: 0, animation: "fadeIn 1.6s ease both" },
  ayahRef: { fontSize: "clamp(12px, 3vw, 13px)", color: "#8e2a1f", letterSpacing: "0.08em", margin: 0, fontWeight: 600 },

  /* CHARACTER CARD */
  characterWrap: { display: "flex", flexDirection: "column", alignItems: "center", gap: 20, width: "100%", textAlign: "center" },
  characterKicker: { fontSize: "clamp(12px, 3vw, 14px)", color: "#8e2a1f", letterSpacing: "0.2em", textTransform: "uppercase", margin: 0, fontWeight: 600 },
  characterCard: {
    width: "100%",
    maxWidth: 480,
    background: "#fbf6ec",
  ...
  characterName: { fontFamily: "'Cormorant Garamond', 'Iowan Old Style', Georgia, serif", fontSize: "clamp(24px, 6vw, 32px)", color: "#8e2a1f", margin: 0, letterSpacing: "0.02em", fontWeight: 600 },
  characterDivider: { width: 60, height: 1, background: "#c0392b", opacity: 0.5, margin: "0 auto" },
  characterLabel: { fontSize: "clamp(11px, 2.8vw, 12px)", color: "#8e2a1f", letterSpacing: "0.15em", textTransform: "uppercase", margin: 0, fontWeight: 600, textAlign: "left" },
  chipRow: { display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "flex-start" },
  chip: { padding: "6px 14px", fontSize: "clamp(12px, 3vw, 13px)", background: "#c0392b", color: "#fffdf7", borderRadius: 999, fontStyle: "italic", letterSpacing: "0.02em" },
  chipAlt: { padding: "6px 14px", fontSize: "clamp(12px, 3vw, 13px)", background: "transparent", color: "#8e2a1f", border: "1px solid #c0392b", borderRadius: 999, fontStyle: "italic", letterSpacing: "0.02em" },
  characterAbility: { fontSize: "clamp(13px, 3.2vw, 15px)", color: "#4a4a4a", lineHeight: 1.6, margin: "8px 0 0", textAlign: "left", fontStyle: "italic" },
  characterCaption: { fontSize: "clamp(15px, 3.8vw, 18px)", color: "#8e2a1f", fontStyle: "italic", margin: 0, animation: "fadeIn 1.2s ease both" },

  /* FROM THIS TO THIS */
  transformWrap: { display: "flex", flexDirection: "column", alignItems: "center", gap: 24, width: "100%" },
  transformRow: { display: "flex", alignItems: "center", justifyContent: "center", gap: "clamp(6px, 2vw, 20px)", flexWrap: "nowrap", width: "100%" },
  imageCol: { flex: "1 1 0", minWidth: 0, display: "flex", justifyContent: "center" },
  transformImg: { width: "100%", maxWidth: 200, aspectRatio: "1 / 1", objectFit: "cover", borderRadius: 14, boxShadow: "0 12px 28px rgba(0,0,0,0.22)", border: "3px solid #fffdf7", background: "#eaddc4", display: "block" },
  arrowCol: { flex: "0 0 auto", display: "flex", alignItems: "center", justifyContent: "center" },
  arrowSvg: { animation: "arrowSlide 1.8s ease-in-out infinite", maxWidth: "100%", height: "auto" },
  transformCaption: { fontSize: "clamp(15px, 3.8vw, 19px)", fontStyle: "italic", color: "#2b2b2b", textAlign: "center", margin: 0, animation: "fadeIn 1.2s ease both" },

  /* FRIENDS */
  friendsWrap: { display: "flex", flexDirection: "column", alignItems: "center", gap: 22, width: "100%", textAlign: "center" },
  friendsIntro: { fontSize: "clamp(16px, 4vw, 20px)", fontStyle: "italic", color: "#2b2b2b", lineHeight: 1.6, margin: 0, maxWidth: 500, animation: "fadeIn 0.8s ease both" },
  friendsList: { display: "flex", flexDirection: "column", gap: 12, width: "100%", maxWidth: 500 },
  friendNote: { fontSize: "clamp(13px, 3.4vw, 15px)", lineHeight: 1.6, color: "#4a4a4a", fontStyle: "italic", margin: 0, padding: "12px 16px", background: "#fbf6ec", borderRadius: 12, borderLeft: "3px solid #c0392b", textAlign: "left", animation: "fadeIn 1s ease both" },
  ...
  deenIntro: { fontSize: "clamp(15px, 3.8vw, 19px)", fontStyle: "italic", color: "#2b2b2b", lineHeight: 1.6, margin: 0, maxWidth: 500, animation: "fadeIn 0.8s ease both" },
  tasbeehHolder: { display: "flex", justifyContent: "center", alignItems: "center", width: "min(320px, 80vw)", margin: "4px auto" },
  tasbeehSvg: { overflow: "visible", width: "100%", height: "auto", display: "block" },
  deenDua: { fontSize: "clamp(13px, 3.5vw, 15px)", fontStyle: "italic", color: "#8e2a1f", margin: 0, maxWidth: 480, lineHeight: 1.6, animation: "fadeIn 1.4s ease both" },

  /* ISTIKHARA */
  istikharaWrap: { display: "flex", flexDirection: "column", alignItems: "center", gap: 16, width: "100%", textAlign: "center" },
  istikharaKicker: { fontSize: "clamp(13px, 3.4vw, 15px)", color: "#8e2a1f", letterSpacing: "0.1em", textTransform: "uppercase", margin: 0, fontWeight: 600 },
  istikharaLine: { fontSize: "clamp(14px, 3.6vw, 17px)", fontStyle: "italic", color: "#2b2b2b", lineHeight: 1.75, margin: 0, maxWidth: 520, animation: "fadeIn 1s ease both" },
  istikharaHadithBox: {
    width: "100%",
    maxWidth: 460,
    background: "#fbf6ec",
  ...
  istikharaHadithText: { fontSize: "clamp(13px, 3.2vw, 14px)", fontStyle: "italic", color: "#4a4a4a", margin: 0, lineHeight: 1.6 },
  istikharaHadithRef: { fontSize: "clamp(11px, 2.8vw, 12px)", color: "#8e2a1f", letterSpacing: "0.04em", margin: 0, fontWeight: 600 },

  /* SADAGI */
  sadagiWrap: { display: "flex", flexDirection: "column", alignItems: "center", gap: 12, width: "100%", textAlign: "center" },
  calligraphySvg: { width: "min(320px, 78vw)", maxWidth: 340, marginBottom: 6, display: "block" },
  sadagiLine: { fontSize: "clamp(15px, 3.8vw, 19px)", fontStyle: "italic", color: "#2b2b2b", lineHeight: 1.7, margin: 0, maxWidth: 520, animation: "fadeIn 1.2s ease both" },
  ...
  proposalArabic: { fontFamily: "'Scheherazade New', 'Amiri', 'Noto Naskh Arabic', serif", fontSize: "clamp(14px, 3.6vw, 18px)", color: "#8e2a1f", direction: "rtl", lineHeight: 1.9, margin: 0, maxWidth: 500, animation: "bismillahGlow 4s ease-in-out infinite" },
  proposalLine: { fontSize: "clamp(17px, 4.4vw, 23px)", fontStyle: "italic", color: "#2b2b2b", lineHeight: 1.6, margin: 0, maxWidth: 520, animation: "fadeIn 1s ease both" },
  thinkingMsg: { fontSize: "clamp(13px, 3.4vw, 15px)", fontStyle: "italic", color: "#8e2a1f", margin: 0, maxWidth: 440, lineHeight: 1.6, animation: "fadeIn 0.6s ease both" },
  proposalBtns: { display: "flex", gap: 12, marginTop: 8, flexWrap: "wrap", justifyContent: "center", alignItems: "center", width: "100%" },
  yesBtn: {
    padding: "14px clamp(28px, 8vw, 44px)",
  ...
  thinkBtn: {
    padding: "14px clamp(22px, 6vw, 32px)",
  ...
  yesBigArabic: { fontFamily: "'Scheherazade New', 'Amiri', 'Noto Naskh Arabic', serif", fontSize: "clamp(28px, 7vw, 42px)", color: "#8e2a1f", margin: 0, animation: "bismillahGlow 2.5s ease-in-out infinite" },
  yesLine: { fontSize: "clamp(16px, 4vw, 21px)", fontStyle: "italic", color: "#2b2b2b", margin: 0, maxWidth: 500, lineHeight: 1.6 },
  yesSub: { fontSize: "clamp(13px, 3.5vw, 15px)", fontStyle: "italic", color: "#8e2a1f", margin: 0, maxWidth: 480, lineHeight: 1.6 },
  yesDua: { fontFamily: "'Scheherazade New', 'Amiri', serif", fontSize: "clamp(14px, 3.6vw, 18px)", color: "#8e2a1f", direction: "rtl", lineHeight: 1.9, margin: "6px 0 0", maxWidth: 500 },
  yesFooterLine: { fontSize: "clamp(12px, 3vw, 13px)", fontStyle: "italic", color: "#6b6b6b", margin: "4px 0 0", maxWidth: 460, lineHeight: 1.6 },
  acceptedFooter: {
    position: "fixed",
    bottom: 12,
    left: 0,
  ...
  /* STEP */
  step: { display: "flex", flexDirection: "column", alignItems: "center", gap: "clamp(24px, 5vw, 36px)", width: "100%", animation: "fadeIn 0.5s ease" },
  stepText: { fontSize: "clamp(18px, 4.5vw, 26px)", lineHeight: 1.6, color: "#2b2b2b", textAlign: "center", fontStyle: "italic", letterSpacing: "0.01em", maxWidth: 560, width: "100%" },
  nextBtn: {
    padding: "14px clamp(28px, 8vw, 42px)",
